...
public abstract class AbstractBaseRDBMSService<
  ...
  public AbstractBaseRDBMSService(
  ...
  public List<DTO> findItemsByIds(List<ID> id) {
    return getRepository().findAllById(id).stream()
  ...
  public List<DTO> findHistoriesById(ID id) {
    Revisions<Integer, DAO> revisions =
        serviceFactory.getRevisionRepository(getDaoClass()).findRevisions(id);
