...
public class StateMachineBuilderWrapper<DOMAIN extends FSMSupport<S>, T extends BaseStateMachine<DOMAIN, T, S, E, C>, S extends Enum<S>, E extends Enum<E>, C extends FSMContext<DOMAIN>> {
    ...
    public StateMachineBuilderWrapper(FSMDefinition<T, DOMAIN, S, E, C> smDefinition, ServiceFactory serviceFactory, ApplicationContext ctx) {
        ...
        this.builder = StateMachineBuilderFactory.create(smDefinition.getStateMachineClass(), smDefinition.getStateClass(), smDefinition.getEventClass(),
                smDefinition.getContextClass(), ApplicationContext.class, ServiceFactory.class);
    ...
    void defineStateMachine() {
        getTransitionDefinitions().forEach(def -> {
            On<T, S, E, C> on = builder.externalTransition().from(EnumUtils.getEnum(stateMachineDefinition.getStateClass(), def.getFrom()))
    ...
    void generateStateMachineDefinition() {
        transitionDefinitions = Arrays
                .stream(Objects
                        .requireNonNull(AnnotationUtils.findAnnotation(stateMachineDefinition.getStateMachineClass(), Transit.Transitions.class)).values())
                .filter(Transit::enabled)
                .flatMap(transit -> Arrays.stream(transit.from())
                        .map(from -> new TransitDefinition().withFrom(from).withOn(transit.on()).withTo(transit.to()).withComment(transit.comment())
                                .withImplicit(transit.implicit()).withEnabled(transit.enabled()).withWhenMvel(transit.whenMvel())))
                .peek(transitDef -> transitDef.setTo(StringUtils.defaultIfEmpty(transitDef.getTo(), transitDef.getFrom()))).collect(Collectors.toList());
    }
}
