...
public class DocumentConfiguration {
    ...
    public Docket defaultApi2() {
        ...
        return new Docket(DocumentationType.SWAGGER_2)
                .apiInfo(new ApiInfoBuilder().description("# BUNNY DDD Restful Api Document").termsOfServiceUrl("http://www.xx.com/")
                        .contact(new Contact("qq", "http://www.qq.com", "xx@qq.com")).version("1.0").build())
                // 分组名称
                .groupName("default").select().apis(RequestHandlerSelectors.basePackage("org.springframework").negate()).paths(PathSelectors.any()).build()
                .pathMapping("/").genericModelSubstitutes(PageableData.class)
